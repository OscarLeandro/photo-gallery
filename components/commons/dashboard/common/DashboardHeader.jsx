...
import { useImageInfo } from "../../../../context/imageContext";
...
export default function DashboardHeader({ setMobileMenuOpen }) {
  ...
  const useMutationPost = useMutation(body => postImage(`${BASE_URL}/api/images`, body),{
    onSuccess: data => {
      const oldImages = queryClient.getQueryData([KEY_IMAGES]);
      console.log('data',data)
      //console.log('oldImages',oldImages)
      queryClient.setQueryData([KEY_IMAGES], [...oldImages, data])
    }
  })
  ...
  async function onFileChange({target}){
    ...
    const cloudUrl = await uploadImage(file,file.name,'ImageUsers' )
    ...
    try {
      ...
      useMutationPost.mutate(newImage)
    } catch (error) {
      console.log(error);
    }
